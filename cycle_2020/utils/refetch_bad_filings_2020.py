import os, logging
from cycle_2020.utils.loader import download_filings
    ...
    with open(filename) as f:
        try:
            if f.readlines(1)[0] == '<!DOCTYPE HTML PUBLIC "-//W3C//DTD HTML 4.01 Transitional//EN"\n':
                logger.info("File {} isn't a csv, FEC site is returning empty web pages".format(file))
                return False
            else:
                return True
...
def recheck_existing_files(filing_dir=filing_dir):
    #find unreadable files and redownload them
    try:
...
if __name__ == '__main__':
    unreadable_files = recheck_existing_files(filing_dir)
    download_filings(unreadable_files, filing_dir)
