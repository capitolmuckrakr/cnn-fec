...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write("looking for filings for period {}-{}".format(start_date, end_date))
        filings = loader.get_filing_list(start_date, end_date)
        if not filings:
            self.stdout.write(self.style.WARNING("failed to find any filings for period {}-{}".format(start_date, end_date)))
